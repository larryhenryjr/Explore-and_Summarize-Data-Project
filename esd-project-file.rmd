...
loandata <-
  read.csv("https://s3.amazonaws.com/udacity-hosted-downloads/ud651/prosperLoanData.csv"
  )
# '~/My Projects/Udacity/udacity-projects/Project Explore and Summarize Data/csv
#   /prosperLoanData.csv')
...
```{r echo=FALSE, message=FALSE, warning=FALSE, out.width=20, df_structure}
str(loandata)
```
...
```{r, echo = TRUE, message=FALSE, warning=FALSE, credit_score_conversion}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Subsetting}
...
loandata <- subset(loandata,
                   select = c(LoanNumber, LoanOriginalAmount, LoanOriginationDate, LoanOriginationQuarter,
                              ...
                              CreditScore, CreditScoreRangeLower, CreditScoreRangeUpper, InquiriesLast6Months,
...
breaks=c(0,1,2,3,4,5,10,15,25)
ggplot(data = subset(loandata, is.na(loandata$InquiriesLast6Months)!=TRUE),
       aes(x=InquiriesLast6Months)) +
  # stat_density(aes(y=..count..), color = 'black', fill = '#96ADBD', alpha=.8) +
  geom_histogram(bins = 25, binwidth = 0.1, color = 'black', fill = '#96ADBD') +
...
ggplot(data = subset(loandata, ProsperRating != ""),
       aes(x = ProsperRating, y = CreditScore)) +
  geom_boxplot(color = '#425E72') +
  labs(caption = "*NA excluded") +
  theme_pubr() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
